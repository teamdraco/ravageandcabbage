	...
	public Iterable<ModelRenderer> getParts() {
	 ...
	 public void setRotationAngles(RCRavagerEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		 ...
		 if (entityIn.isBeingRidden()) {
			 this.legBackRight.rotateAngleX = MathHelper.cos((limbSwing / 3) * 0.6662F) * f;
			 this.legBackLeft.rotateAngleX = MathHelper.cos((limbSwing / 3) + (float)Math.PI) * f;
			 this.legFrontRight.rotateAngleX = MathHelper.cos((limbSwing / 3) + (float)Math.PI) * f;
			 this.legFrontLeft.rotateAngleX = MathHelper.cos((limbSwing / 3) * 0.6662F) * f;
		 }
	 }
