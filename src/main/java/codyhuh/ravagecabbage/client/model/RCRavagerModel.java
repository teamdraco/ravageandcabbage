...
public class RCRavagerModel extends AgeableListModel<RCRavagerEntity> {
	...
	public RCRavagerModel(ModelPart p_170889_) {
		this.root = p_170889_;
	...
	public void setupAnim(RCRavagerEntity p_103626_, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		 ...
		 if (p_103626_.isVehicle()) {
			 this.rightHindLeg.xRot = (Mth.cos((limbSwing / 4) * 0.6662F) * f);
			 this.leftHindLeg.xRot = (Mth.cos((limbSwing / 4) * 0.6662F + (float)Math.PI) * f);
			 this.rightFrontLeg.xRot = (Mth.cos((limbSwing / 4) * 0.6662F + (float)Math.PI) * f);
			 this.leftFrontLeg.xRot = (Mth.cos((limbSwing / 4) * 0.6662F) * f);
		 }
	}

	@SuppressWarnings("unused")
	public void prepareMobModel(RCRavagerEntity p_103621_, float p_103622_, float p_103623_, float p_103624_) {
